...
A shared, officer-maintained suggested 20-man raid comp, built on top of Bench (`src/lib/bench.ts`), which ranks Death Analysis's qualified raiders by a combined score of Pull Score (this site's own recency-weighted per-pull score — see [Parse Analysis](#parse-analysis) — not WCL's Parse) and death percentile. Bench has no page of its own — it's the scoring engine behind Raid Comp's Configuration Settings.
...
Explains and displays **Pull Score**, this site's own recency-weighted per-pull performance score, alongside WCL's own **Parse** for reference. The two are deliberately kept distinct everywhere: Pull Score feeds Raid Comp's scoring, Parse never does.

- **Pull Score**: computed from every qualifying Heroic pull (kills *and* wipes, at least 30 seconds — unlike WCL Parse, whose public API only exposes kills) in `src/lib/pull-scores.ts`. On each pull, a raider's DPS (tanks/DPS) or HPS (healers) is compared to the median of everyone in that role on the same pull: `pull_score = clamp(0, 100, 50 × amount ÷ role_median)` — 50 is typical for the role that pull, 100 is twice typical or better. A raider's overall Pull Score is the weighted average of their own pulls in their effective role, with the same 30-day recency half-life as Death Analysis. Needs at least `PULL_SCORE_MIN_PULLS` (5) scored pulls in a role before it's shown.
- **Page** (`/parse-analysis`, Tools menu, guild members): a "How this works" explainer plus four role tables (Tanks/Healers/Ranged/Melee) — Pull Score, pulls, kills, and WCL Parse (kills, weighted avg / all-time median) as secondary reference columns, plus a 14-day trend. Expanding a raider's row shows their pull history (date, boss, kill/wipe + boss %, DPS/HPS, role median, Pull Score, WCL Parse, current recency weight, and a link to the Warcraft Logs fight) via the shared `src/components/PullHistoryTable.astro`. The same panel (summary line + history table) appears on each raider's profile as "Pull Score History".
- **Sync**: `refreshPullScores` runs chained after Bench parses in `/api/cron/refresh-logs` and from the "Refresh Logs Data" button, syncing a few fights per report per tick (`pull_score_reports` is the cursor) — a per-pull `table`/`playerDetails` query is heavier WCL load than anything else on the site, so it gets its own smaller budget. A separate rankings pass fills in WCL's percentile for kills only, once fights finish syncing and while the report is recent enough that WCL has likely computed rankings.
- **Visibility**: same as Death Analysis — every guild member sees everyone's Pull Score and WCL Parse; no officer-only view.
- **Tables**: `pull_score_pulls` (one row per raider per pull) and `pull_score_reports` (per-report sync cursor) in `migrations/0103_pull_scores.sql` and `migrations/0104_pull_score_rankings_attempted_at.sql`.
...
| `pull_score_pulls` / `pull_score_reports` | Parse Analysis: one row per raider per qualifying pull (amount, role median, Pull Score, WCL bracketPercent on kills), and the per-report fights/rankings sync cursor (see [Parse Analysis](#parse-analysis)) |
