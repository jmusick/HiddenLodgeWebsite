...
- `parseAnalysis` — `/parse-analysis` (Tools menu, guild members) and the raider profile's "Pull Score History" panel. Also requires `deathAnalysis`. Note this gates the *page* only — Pull Score's own sync and Raid Comp's scoring run whenever `deathAnalysis` is on, regardless of this flag. See [Parse Analysis](#parse-analysis).
...
The "Tools" nav dropdown isn't gated as a whole; each entry checks its own flag. It currently holds Gear Summary (`/raiders/gear-summary`, guild members), Professions (logged-in users, `tools`), Loot History (logged-in users, `tools`), Death Analysis (guild members, `deathAnalysis`), Mechanics Analysis (guild members, `deathAnalysis` + `mechanicsAnalysis`), Raid Composition (guild members, `deathAnalysis` + `raidComp`), and Parse Analysis (guild members, `deathAnalysis` + `parseAnalysis`).
...
- Parse Analysis (`/parse-analysis`, Tools menu, guild members) — this site's own recency-weighted Pull Score per raider/role, alongside WCL's own Parse for reference, with per-raider pull history; see [Parse Analysis](#parse-analysis)
...
- **Scope**: reports whose zone is **The Venomous Abyss** and that overlap a **Thursday/Friday 9pm–midnight Eastern** window by at least 30 minutes (DST-aware). Only **Heroic and Mythic** boss pulls count, and only pulls lasting **30+ seconds** (`MIN_FIGHT_DURATION_MS`) — shorter ones are treated as an instant wipe rather than a real attempt. Rolling **last 90 days**; older rows are pruned on each sync.
...
- **Scoring** (same formula as Season 1): weighted score = (4 × first deaths + 3 × second + 2 × third + 1 × fourth) / pulls attended; only the first 4 deaths per pull count; only raiders with 10+ pulls across 2+ reports (`DEATH_ANALYSIS_MIN_PULLS` / `DEATH_ANALYSIS_MIN_REPORTS`; Season 1 was 5 pulls, no report minimum) are ranked and averaged — everyone else is hidden from the table; more than 25% above the qualified average is highlighted.
...
- **Death links**: each raider row on `/death-analysis` expands (click "N deaths logged") into a list of that raider's individual counted deaths — raid night, boss, death order, and time into the pull — each linking straight to `warcraftlogs.com/reports/<code>#fight=<id>&type=deaths&death=<position>` to jump directly to that death's moment (WCL's own deep-link param). Backed by `death_analysis_events`, populated alongside the aggregate stats during sync.
- **Tables**: `death_analysis_reports`, `death_analysis_stats`, `death_analysis_night_overrides` (`migrations/0074_death_analysis.sql`), `death_analysis_events` (`migrations/0078_death_analysis_events.sql`).
...
A shared, officer-maintained suggested 20-man raid comp, built on top of Bench (`src/lib/bench.ts`), which ranks Death Analysis's qualified raiders by a combined score of Pull Score (WCL's own percentile parse, decay-weighted by recency instead of WCL's flat all-time average — see [Parse Analysis](#parse-analysis)) and death percentile. Bench has no page of its own — it's the scoring engine behind Raid Comp's Configuration Settings.
...
Explains and displays **Pull Score**, alongside WCL's own **Parse** for reference. The two share the same underlying number — WCL's own per-kill percentile parse, already normalized for spec/boss/item level — the difference is only how they're averaged. Pull Score is deliberately kept distinct in the UI (it feeds Raid Comp's scoring, WCL Parse never does), but under the hood Pull Score literally *is* WCL Parse, decayed by recency instead of averaged flat over all time.

- **Pull Score**: computed in `src/lib/pull-scores.ts`. WCL's `bracketPercent` for each of a raider's counted Heroic kills is stored per pull, then averaged with a decay weight — a kill from 30 days ago counts half as much as one from today (the same half-life Death Analysis uses). Only **kills** score; Warcraft Logs' public API has no percentile for wipes, so wipes are still synced and shown (for attendance context, and in each raider's pull history) but never move the score. An earlier version tried to score wipes too, comparing a raider's raw DPS/HPS to the median of everyone in their role on that one pull — that was dropped because it has no reference point outside the raid group: a raider who's the only one of their spec in the guild can never score above or below 50 under a same-spec median, since the median *is* their own value. WCL's percentile doesn't have that problem, since it's compared against a worldwide population. Needs at least `PULL_SCORE_MIN_PULLS` (5) scored kills in a role before it's shown.
- **Page** (`/parse-analysis`, Tools menu, guild members): a "How this works" explainer plus four role tables (Tanks/Healers/Ranged/Melee) — Pull Score, pulls attended, kills, and WCL Parse (median, the old flat all-time number Pull Score replaces) as a secondary reference column, plus a 14-day trend. Expanding a raider's row shows their pull history (date, boss, kill/wipe + boss %, DPS/HPS, role median — display context only, no longer feeds the score — Parse, current recency weight, and a link to the Warcraft Logs fight) via the shared `src/components/PullHistoryTable.astro`. The same panel (summary line + history table) appears on each raider's profile as "Pull Score History".
- **Sync**: `refreshPullScores` runs chained after Bench parses in `/api/cron/refresh-logs` and from the "Refresh Logs Data" button, syncing a few fights per report per tick (`pull_score_reports` is the cursor) — a per-pull `table`/`playerDetails` query is heavier WCL load than anything else on the site, so it gets its own smaller budget. A separate rankings pass writes each kill's `bracketPercent` straight into `pull_score`, once fights finish syncing; the first attempt for a report runs regardless of its age, and only a retry after an empty first attempt is limited to reports still within a few days of the raid night (WCL computes rankings with a lag).
- **Visibility**: same as Death Analysis — every guild member sees everyone's Pull Score and WCL Parse; no officer-only view.
- **Tables**: `pull_score_pulls` (one row per raider per pull — `pull_score` nullable, since only kills WCL has ranked get one) and `pull_score_reports` (per-report sync cursor) in `migrations/0103_pull_scores.sql`, `0104_pull_score_rankings_attempted_at.sql`, and `0105_pull_score_nullable.sql`.
...
| `/parse-analysis` | Yes + Guild Member | Pull Score vs WCL Parse, per role, with per-raider pull history (see [Parse Analysis](#parse-analysis)) |
...
| `/api/admin/bench/refresh` | POST | Officer: run a Bench parse (and mechanic-role) refresh, plus a Pull Score refresh, now; redirects to `/raid-composition` |
...
| `pull_score_pulls` / `pull_score_reports` | Parse Analysis: one row per raider per qualifying pull (DPS/HPS amount and role median for display; `pull_score` is WCL's `bracketPercent`, null except on ranked kills), and the per-report fights/rankings sync cursor (see [Parse Analysis](#parse-analysis)) |
