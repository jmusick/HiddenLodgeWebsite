...
async function fetchAttendanceReportStats(accessToken: string, reportCode: string, ownership: WclCharacterLookup) {
  const s1EncounterIds = await getMidnightS1EncounterIds(accessToken);
  // Fail open: if raid encounter metadata is unavailable, count deaths for all boss encounters.
  return fetchReportFightStats(accessToken, reportCode, ownership, (fight) =>
    s1EncounterIds.size === 0 || s1EncounterIds.has(Math.floor(Number(fight.encounterID ?? 0)))
  );
}
...
async function syncAttendanceOccurrence(
  ...
  if (overrideReportCode) {
    const details = await fetchAttendanceReportStats(accessToken, overrideReportCode, ownership);
  ...
  for (const candidate of candidates.slice(0, 3)) {
    const details = await fetchAttendanceReportStats(accessToken, candidate.code, ownership);
...
export async function importAttendanceFromReportCode(
  ...
  try {
    details = await fetchAttendanceReportStats(accessToken, code, ownership);
...
export async function getAttendanceSummaryMap(
  ...
  for (const row of participantsResult.results ?? []) {
    ...
    const ownerKey = ownership.ownerKeyByCharId.get(row.blizzard_char_id) ?? characterOwnerKey(null, row.blizzard_char_id);
  ...
  const targetOwnerKeys = [...new Set(targetCharIds.map((blizzardCharId) => ownership.ownerKeyByCharId.get(blizzardCharId) ?? characterOwnerKey(null, blizzardCharId)))];
  const includeBreakdownForOwners = new Set([...includeBreakdownFor].map((blizzardCharId) => ownership.ownerKeyByCharId.get(blizzardCharId) ?? characterOwnerKey(null, blizzardCharId)));
