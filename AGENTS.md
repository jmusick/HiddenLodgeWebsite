...
- **Feature flags**: `src/lib/feature-flags.ts` exports `FEATURE_FLAGS`, a plain object of booleans (one per major feature: `rosterTeams`, `raidSignups`, `attendance`, `deathAnalysis`, `applications`, `feedback`, `tools`, `sim`). A feature that's temporarily disabled but not removed is gated everywhere it surfaces:
